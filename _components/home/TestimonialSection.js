...
const TestimonialSlider = () => {
  ...
  return (
    <div
      className="w-full bg-gradient-to-br from-slate-50 to-slate-100 py-16 px-4"
    ...
    >
      ...
      onMouseLeave={() => setIsHovered(false)} // Resume on leave
        >
          ...
          <div className="overflow-hidden">
            ...
            >
              {testimonials.map((testimonial) => (
                <div key={testimonial.id} className="w-1/4 flex-shrink-0 px-2">
                  <div className="bg-white rounded-xl shadow-md border border-slate-200 p-5 h-full hover:shadow-lg transition-shadow duration-300">
                    ...
                    <div className="mb-3">
                      <p className="text-slate-700 text-sm leading-relaxed">
                        {/* Truncate review text if not expanded */}
                        {expandedCard === testimonial.id
                          ? testimonial.review
                          : `${testimonial.review.substring(0, 100)}${testimonial.review.length > 100 ? '...' : ''}`
                        }
                      </p>
                    </div>
                    ...
                    {testimonial.review.length > 100 && (
                      ...
                      >
                        {expandedCard === testimonial.id ? 'Read less' : 'Read more'}
                        <svg
          ...
          <button
            onClick={prevSlide}
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white/80 backdrop-blur-sm rounded-full p-2 border border-slate-200 hover:bg-white shadow-md transition-all duration-300 -ml-4 z-10"
          ...
          <button
            onClick={nextSlide}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white/80 backdrop-blur-sm rounded-full p-2 border border-slate-200 hover:bg-white shadow-md transition-all duration-300 -mr-4 z-10"
            ...
            {(testimonials).map((_, index) => (
              <button
                ...
                onClick={() => {
                  ...
                  setExpandedCard(null); // Collapse any expanded card on slide change
                }}
